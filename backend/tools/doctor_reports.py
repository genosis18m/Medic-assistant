...
async def send_report_to_telegram(
    ...
    """
    Generate PDF report and send to Doctor via Telegram using Chat ID or Phone Number.
    """
    try:
        from services.telegram_service import generate_pdf_report, send_telegram_pdf
        ...
        with get_session() as session:
            ...
            # For weekly report, go back 7 days
            ...
            # Fetch appointments
            appointments = session.exec(select(Appointment).where(
                Appointment.doctor_id == doctor_id,
            ...
            for apt in appointments:
                 apt_list.append({
                    "time": f"{apt.appointment_date.strftime('%Y-%m-%d')} {apt.appointment_time.strftime('%H:%M')}",
                    "patient_name": apt.patient_name,
                    "reason": apt.reason
                })
            
            # Generate PDF
            pdf_buffer = generate_pdf_report(doctor.name, apt_list, query_date.strftime("%Y-%m-%d"))
            
            # Send via Telegram Service (handles phone resolution)
            result = await send_telegram_pdf(
                chat_id=chat_id, 
                phone_number=phone_number, 
                pdf_file=pdf_buffer, 
                filename=f"Report_{report_type}_{doctor_id}.pdf"
            )
            return result
